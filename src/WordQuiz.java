...
public class WordQuiz {
	...
	private int makeExample(int ex[], int answerIndex) {//보기 구성하기
		...
		for(int i=0;i<n.length;i++) {
			...
			}while(index == answerIndex || exists(n,index));  //정답과 같은 index번호이거나 n배열에 이미 들어있는 값이면 사용불가 판정
			n[i] = index;
		}
		for(int i=0;i<n.length;i++) {
			ex[i] = n[i]; //배열의 복사
	...
	public void run() { //게임 시작 하기
		System.out.println("[" + name + "]" + "게임 시작 | -1을 입력하면 게임이 종료됩니다");
